...
if rank == 0:
    ...
    print(f"World: {world_size} GPUs, {world_size // torch.cuda.device_count()} nodes")
    print(f"GPU: {torch.cuda.get_device_name(0)} ({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)")

def save_checkpoint_and_exit(signum, frame):
    if rank == 0: print(f"\n[SIGUSR1] Checkpoint signal received")
...
if rank == 0: print(f"Loading model (Unsloth 4-bit, world_size={world_size})...")
...
for module in model.modules():
    ...
    def make_patched_moe(mod):
        def patched_moe(_self, hidden_states, topk_indices, topk_weights):
            ...
            hidden_states = hidden_states.repeat_interleave(topk_indices.shape[-1], dim=0)
            ...
            for i, expert_layer in enumerate(mod.experts):
                ...
                if expert_mask.numel() == 0: continue
                ...
                if weighted_output.dtype != dtype: weighted_output = weighted_output.to(dtype)
...
if rank == 0: print(f"  MoE patch: {_patched_count} layers (rank {rank})")
...
def fmt(ex):
    ...
    if not msgs: msgs = [{"role":"user","content":ex.get("prompt","?")},{"role":"assistant","content":ex.get("answer","?")}]
    return {"text": tok.apply_chat_template(msgs, tokenize=False, add_generation_prompt=False)}
ds = ds.map(fmt)
if rank == 0: print(f"Data: {len(ds)} examples")

from trl import SFTTrainer, SFTConfig
trainer = SFTTrainer(
    model=model,
    args=SFTConfig(output_dir=OUT_PATH, max_steps=500, per_device_train_batch_size=1,
        gradient_accumulation_steps=4, learning_rate=2e-4, max_seq_length=2048,
        warmup_steps=50, logging_steps=10, save_strategy="steps", save_steps=100,
        save_total_limit=3, bf16=True, remove_unused_columns=False, report_to="none",
        dataloader_num_workers=0, packing=True, ddp_find_unused_parameters=False),
    train_dataset=ds, processing_class=tok,
)

if rank == 0: print(f"TRAINING: {world_size} GPUs, effective_batch={world_size*4}, 500 steps")
resume = RESUME_CHECKPOINT if RESUME_CHECKPOINT else True
...
try:
    ...
    if rank == 0:
        ...
        ckpt = os.path.join(OUT_PATH, "checkpoint-latest")
        trainer.save_model(ckpt); tok.save_pretrained(ckpt)
...
if rank == 0:
    ...
    manifest = {"model":MODEL_PATH,"data":DATA_PATH,"num_examples":len(ds),
        "max_steps":500,"batch_size":1,"grad_accum":4,"effective_batch":world_size*4,
        "world_size":world_size,"lr":2e-4,"max_seq_len":2048,"elapsed_min":elapsed,
        "framework":"unsloth-4bit-qlora-multi-node-ddp"}
    with open(os.path.join(OUT_PATH,"manifest.json"),"w") as f: json.dump(manifest,f,indent=2)
